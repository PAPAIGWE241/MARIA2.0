...
async function gandu() {
  try {
    ...
    if (gnome && gnome.trim().toLowerCase() !== lund.toLowerCase()) {
      ...
      console.log(`${endi}`)
      console.log(chalk.bgBlack(chalk.redBright('initializing Maria Bot')))
    }
  } catch (error) {
    console.error('Error:', error)
  }
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
    ...
    const msg = `Hai🤩 ${name}, Congrats you have successfully deployed MARIA-BOT BUDDY\nJoin my support Group for any Query\n https://chat.whatsapp.com/Lab7eeUSWBxGL4k2ku73gK`
